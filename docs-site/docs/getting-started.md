...
2. **Azure resources** → Full setup tab → **+** → Resource groups → select your demo resource group (for example, `rg-webstore-prod`) ([docs](https://sre.azure.com/docs/get-started/create-and-setup#add-azure-resource-access))
...
| Secrets | Variables |
|---------|-----------|
| `AZURE_CLIENT_ID` — app registration client ID | `CONTAINER_APP_NAME` — e.g. `ca-webstore-prod` |
| `AZURE_TENANT_ID` — Entra ID tenant | `RESOURCE_GROUP` — e.g. `rg-webstore-prod` |
| `AZURE_SUBSCRIPTION_ID` — target subscription | |
| | `APP_INSIGHTS_NAME` — e.g. `appi-webstore-prod` |
