...
> ⚠️ **Decommissioned:** The AKS infrastructure (`rg-webstore-aks`) has been deleted to reduce Azure costs. The scenario docs and IaC are preserved as reference for future re-deployment.

> ℹ️ **Status:** AKS Bicep, Kubernetes manifests, monitoring alert, runbook, and documentation are preserved as reference. The break/reset workflows have been removed. Bootstrap steps (Azure provisioning and container image build/publish) are manual — see [Setup](#setup) below.
...
│                                                  │
│  webstore-blue  (same image, stable)   ← 100% traffic    │
│  webstore-green (same image,           ← 0% traffic       │
│                  DEMO_BROKEN_CHECKOUT=true)                │
...
| `k8s/blue-deployment.yaml` | Stable deployment (`acrwebstorestaging.azurecr.io/aks-stub:v1`, `DEMO_BROKEN_CHECKOUT=false`) |
| `k8s/green-deployment.yaml` | Broken deployment (same image, `DEMO_BROKEN_CHECKOUT=true`) |
